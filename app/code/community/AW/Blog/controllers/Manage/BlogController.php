        ...
        if ($data = $this->getRequest()->getPost()) {
            if(isset($_FILES['thumbnail']['name'])&& $_FILES['thumbnail']['name'] != '') {
                try {
                    ...
                    preg_match('/\.[^\.]+$/i',$file,$ext); // get extension
                    $filename = substr($file,0,-(strlen($ext[0]))).'_'.time().$ext[0];
                    if ($uploader->save($path, $filename)) {
                        Mage::log($filename);
                        $data['thumbnail'] = 'blog' . DS . $uploader->getUploadedFileName();
                    }
                ...
                // Unset filename part when image upload field is empty
            }
