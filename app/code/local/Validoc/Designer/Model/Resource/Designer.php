 ...
 * Time: 2:56 PM
 */ 
class Validoc_Designer_Model_Resource_Designer extends Mage_Core_Model_Resource_Db_Abstract
{

    protected function _construct()
    {
        $this->_init('validoc_designer/designer', 'designer_id');
    }
    ...
    protected function __getMediaGallery($object) {
        $select = $this->_getReadAdapter()->select()
            ->from($this->getTable('validoc_designer/designer_image'))
            ->where('designer_id = ?', $object->getId())
...
//            $this->__saveToVideoTable($object);
    ...
    private function __loadImage(Mage_Core_Model_Abstract $object) {
        $select = $this->_getReadAdapter()->select()
            ->from($this->getTable('validoc_designer/designer_image'))
            ->where('designer_id = ?', $object->getId())
            ->order(array('position ASC', 'designer_id'));

        $selectImageToFrontEnd = $this->_getReadAdapter()->select()
            ->from($this->getTable('validoc_designer/designer_image_flag'), array('image','small_image','medium_image','thumbnail'))
            ->where('designer_id = ?', $object->getId());
    ...
    private function __saveToImageTable(Mage_Core_Model_Abstract $object) {
        ...
        if ($_imageList = $object->getData('designer/media_gallery/images')) {
            $_imageList = Zend_Json::decode($_imageList);
            if (is_array($_imageList) and sizeof($_imageList) > 0) {
                $_imageTable = $this->getTable('validoc_designer/designer_image');
                $_adapter = $this->_getWriteAdapter();
                $_adapter->beginTransaction(); // begin transaction
                try {
                    $condition = $this->_getWriteAdapter()->quoteInto('designer_id = ?', $object->getId());
                    $this->_getWriteAdapter()->delete($this->getTable('validoc_designer/designer_image'), $condition);

                    foreach ($_imageList as &$_item) {
                        if (isset($_item['removed']) and $_item['removed'] == '1') {
                            $_adapter->delete($_imageTable, $_adapter->quoteInto('designer_id = ?', $_item['value_id'], 'INTEGER'));
                        } else {
                            $_data = array(
                                'label' => $_item['label'],
                                'file' => $_item['file'],
                    ...
                    $condition = $this->_getWriteAdapter()->quoteInto('designer_id = ?', $object->getId());
                    $this->_getWriteAdapter()->delete( $this->getTable('validoc_designer/designer_image_flag'), $condition );

                    $_imageValues = $object->getData('designer/media_gallery/values');
                    ...
                    if ( !empty($_imageValues) ) {
                        ...
                        $_adapter->insert($this->getTable('validoc_designer/designer_image_flag'), $_data);
                    }
    ...
    public function getImageUrl(Varien_Object $object, $type) {
        $_imageTable = $this->getTable('validoc_designer/designer_image');
        $_imageTableFlag = $this->getTable('validoc_designer/designer_image_flag');
        try {
            $dbselect = $this->_getReadAdapter()->select();
            $dbselect->from(array('image' => $_imageTable));
            $dbselect->join(array('flag' => $_imageTableFlag), 'image.designer_id = flag.designer_id', array('image', 'small_image', 'medium_image', 'thumbnail'));
            $dbselect->where('image.designer_id = ?', $object->getId());
            switch($type) {
